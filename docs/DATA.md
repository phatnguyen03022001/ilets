...
`docs/DATA.md`, `docs/PRODUCT.md`, `docs/BEHAVIOR.md`, `docs/ARCHITECTURE.md`, `docs/DECISIONS.md`, and `docs/catalog/project.json` remain migration artifacts with **authority NONE**. `TASK-0014` resolves only `UNK-006` after complete typed DATA mapping for `FTR-007..FTR-053`; `UNK-007`, `UNK-008`, and `UNK-009` remain OPEN downstream DESIGN blockers and `milestone.scope_state` remains `OPEN`. This task does not cut over canonical ownership, freeze milestone scope, modify IFC/EXT/CAP mappings, materialize exact contracts/providers/storage schema, claim `DOCS_READY`, or declare implementation/assurance/promotion/release readiness.
