...
## Model ([KK-001](decisions.md), [KK-002](decisions.md))
Every knowledge item is an atomic **Knowledge Object** conforming to [knowledge-object-schema.md](knowledge-object-schema.md) v1.0: stable `id`, atomic `definition`, and explicit edges (`requires`, `related_to`). Dependencies are graph edges, not implicit. Decompose until every Skill prerequisite resolves to ≥1 object — mapped in [resolution.md](resolution.md).
